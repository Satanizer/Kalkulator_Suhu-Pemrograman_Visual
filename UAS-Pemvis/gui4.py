...
ASSETS_PATH = OUTPUT_PATH / Path(r"F:\Pemvis\New2\build\assets\frame4")

def relative_to_assets(path: str) -> Path:
    return ASSETS_PATH / Path(path)

def on_button_click(text, input_text):
    if text.isdigit() or (text == "." and "." not in input_text.get()):
        current_text = input_text.get()
        if "°" in current_text:
            input_text.delete(len(current_text) - 2, "end")
        input_text.insert("end", text + "°R")
    elif text == "delete":
        input_text.delete(len(input_text.get()) - 1, "end")
    elif text == "Ac":
        input_text.delete(0, "end")
    elif text == "konversi Celcius":
        reamur = float(input_text.get().replace("°R", ""))
        celcius = 5/4 * reamur
        input_text.delete(0, "end")
        input_text.insert(0, f"{celcius:.2f}°C")
    elif text == "konversi Fahrenheit":
        reamur = float(input_text.get().replace("°R", ""))
        fahrenheit = (9/4 * reamur) + 32
        input_text.delete(0, "end")
        input_text.insert(0, f"{fahrenheit:.2f}°F")
    elif text == "konversi Kelvin":
        reamur = float(input_text.get().replace("°R", ""))
        kelvin = 5/4*reamur + 273.15
        input_text.delete(0, "end")
        input_text.insert(0, f"{kelvin:.2f}°K")
    elif text == "E":
        input_text.delete(0, "end")
...
def show_reamur(window):
    ...
    input_text.place(x=1600, y=363, anchor="ne", width=500)

    buttons = [
        ("button_1.png", ".", (1018.0, 707.0)),
        ("button_2.png", "delete", (602.0, 544.0)),
        ("button_3.png", "konversi Fahrenheit", (1381.0, 870.0)),
        ("button_4.png", "konversi Celcius", (1382.0, 544.0)),
        ("button_5.png", "konversi Kelvin", (1559.0, 542.0)),
    ...
    for image_filename, text, position in buttons:
        ...
        if text == "back":
            ...
            button = Button(
                ...
                borderwidth=0,
                highlightthickness=0,
                command=lambda t=text: on_button_click(t, input_text),
                relief="flat"
            )
